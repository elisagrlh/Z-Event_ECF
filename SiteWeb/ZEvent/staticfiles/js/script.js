...
let app = Vue.createApp({
    ...
    methods: {
        ...
        scrollToAsso() {
            const assoElement = document.getElementById('asso');
            const assoPosition = assoElement.offsetTop;
            const offset = window.innerHeight / 2 - assoElement.offsetHeight / 2; // Calcul pour centrer l'élément
            window.scrollTo({
                top: assoPosition - offset,
                behavior: 'smooth'
            });
        },
        scrollToActus() {
            const actusElement = document.getElementById('actus');
            const actusPosition = actusElement.offsetTop;
            const offset = window.innerHeight / 2 - actusElement.offsetHeight / 2; // Calcul pour centrer l'élément
            window.scrollTo({
                top: actusPosition - offset,
                behavior: 'smooth'
            });
        },
        redirectWWF() {
        ...
        checkMaterialCheckbox() {
            var selectedMaterials = this.live.material.map(material => `${material.label} (${material.brand})`);
            ...
            // Set new selections
            selectedMaterials.forEach(function (materialLabel) {
                for (var i = 0; i < selectElement.options.length; i++) {
                    if (selectElement.options[i].text === materialLabel) {
